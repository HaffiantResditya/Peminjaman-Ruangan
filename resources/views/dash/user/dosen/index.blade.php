    ...
    <div class="flex p-5 flex-1">
        <main class="bg-white min-h-[40vh] flex-1">
            <div class=" mb-10 px-5">
                <p class="font-bold text-blue-500 pb-3">Data Dosen</p>
                <hr>
            </div>
            <div class="flex justify-between items-center gap-5 px-5">
                <a href="{{ route('dash.dosen.tambah') }}"
                    ...
                    @foreach ($payload as $index => $item)
                        <tr class="border h-[60px] items-center font-semibold">
                            ...
                            <td class="flex justify-center items-center gap-3 h-[60px] pr-3">
                                <a href="{{ route('dash.dosen.ubah', ['id' => $item->id]) }}"
        ...
        if (searchInput) {
            searchInput.addEventListener('change', function() {
                const id = setTimeout(() => {
                    ...
                    console.log(key);
                    if (key.length > 0) document.location.href = `/dash/dosen?keyword=${key}`
                    clearTimeout(id);
                }, 300);
            })
        }
