    ...
    <div class="flex p-5 flex-1">
        <main class="bg-white min-h-[40vh] flex-1">
            <div class=" mb-10 px-5">
                <p class="font-bold text-blue-500 pb-3">Pengajuan Masuk</p>
                <hr>
            </div>
                ...
                <table class="w-full text-[12px] lg:text-[16px]">
                    ...
                    @foreach ($payload as $index => $item)
                        <tr class="border h-[60px] items-center font-semibold">
                            ...
                            <td class="flex justify-center items-center gap-3 h-[60px] pe-3">

                                <a href="{{ route('dash.pengajuan.edit', ['id' => $item->id]) }}"
                                    class="h-[35px] flex justify-center items-center bg-blue-500 rounded px-3 gap-2 text-white font-medium"><i
                                        class="fa-solid fa-pen-to-square"></i>Tanggapi</a>
                            </td>
                        </tr>
                    @endforeach
            ...
            searchInput.addEventListener('change', function() {
                const id = setTimeout(() => {
                    ...
                    if (key.length > 0) document.location.href =
                        `/dash/pengajuan/pemakaian-ruangan?keyword=${key}`
                    clearTimeout(id);
                }, 300);
            })
        }
